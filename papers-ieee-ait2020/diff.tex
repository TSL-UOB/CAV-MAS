...
%DIF DEL old.tex   Mon Jan 20 08:56:24 2020
%DIF ADD new.tex   Fri Jan 17 13:27:29 2020
...
%
Our aim is to generate tests that exercise an assertion that requires the AV to avoid collisions with other road users, provided that such road user, a pedestrian in this case, intrudes in the path of the AV in such a way that a collision can be avoided by the AV either by braking or manoeuvring. \DIFaddbegin \DIFadd{This is similar to standardised testing, e.g. Euro-NCAP CPNA-25~\mbox{%DIFAUXCMD
\cite{EURO-NCAP}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend In Fig.~\ref{gridRoad} this is illustrated by the \textit{precondition zone}, i.e.\ the area of interest in which the assertion is activated.
\DIFdelbegin %DIFDELCMD < \todo[inline]{This is similar to standardised testing, e.g. Euro-NCAP CPNA-25~\cite{EURO-NCAP}}
%DIFDELCMD < %%%
\DIFdelend 
...
The second class of behaviours is agency-directed, taking into account the agent's perceptions for strategic planning. Agents are initially walking along the pavement as in constrained random mode. The \textit{proximity} behaviour instructs the agent to cross the road when the AV is within a certain radius. %DIF > 
%DIF > 
\DIFaddbegin \DIFadd{The trigger radius is 15 cells using city-block measure and is the same value for both }\textit{\DIFadd{proximity}}  \DIFadd{and }\textit{\DIFadd{election}} \DIFadd{behaviours.
%DIF > 
}\DIFaddend The \textit{election} behaviour ranks agents within a proximity radius to the AV and elects the agent with the shortest distance to the AV. The major difference between the agent-directed behaviours is that the \textit{election} behaviour will only elect a single agent to cross the road whereas the \textit{proximity} behaviour will result any number of agents crossing the road as long as they are within range.
\DIFdelbegin %DIFDELCMD < \todo[inline]{Specify what the trigger radius is: 15 cells using city-block measure, note that this is a model parameter}
%DIFDELCMD < %%%
\DIFdelend % Thus, the \textit{proximity} behaviour results in agents that will step into the road whenever a vehicle is nearby. Moreover, all nearby pedestrians are drawn to a passing vehicle.
%
...
In an attempt to encourage more realistic agent behaviour, a basic scoring system is used to direct agent behaviour by penalising or rewarding certain actions. In particular, a living cost of one is charged for each elapsed time step to promote shorter tests, and a penalty of five is issued for each time step that an agent spends in the road. %DIF > 
%DIF > 
\DIFaddbegin \DIFadd{This is based on the general observation that most pedestrians do not predominately walk in the road but rather cross over it and hence higher scores are associated with less time spent in the road.
%DIF > 
}\DIFaddend A reward of 100 is given if the agent enters the AV's \textit{precondition zone} More sophisticated scoring systems will be explored in our future work. 
